...
export class Login extends Component {
  ...
  addUsers = users => {
    for (let index = 0; index < users.length; index++) {
      ...
      this.setState({
        users: [...this.state.users, { username: user.username, id: user.id }]
      })
    }
  }
  ...
  validatieUser = event => {
    event.preventDefault()
    if (
      [...this.state.users].find(name => name.username === this.state.username)
    ) {
      ...
      <div>
        ...
        <form onSubmit={event => this.validatieUser(event)}>
          <div class="ui icon input">
            ...
            <button class="ui button" class="ui button" type="submit">
              Submit
            </button>
          </div>
        </form>
        ...
        <div class="ui icon input">
          ...
          <button class="ui button" class="ui button" type="submit">Submit</button>
        </div>
        </form>
      </div>
    )
